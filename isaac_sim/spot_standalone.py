...
# Assign RealSense color/depth camera prims from the earlier discovery
if not ENABLE_REALSENSE:
    print("[spot_standalone] RealSense disabled (ENABLE_REALSENSE=False)")
    REALSENSE_COLOR_CAM = None
    REALSENSE_DEPTH_CAM = None
elif not _rs_cams:
    carb.log_warn(f"[spot_standalone] No Camera prims found under rsd455 — RealSense graphs skipped")
    REALSENSE_COLOR_CAM = None
    REALSENSE_DEPTH_CAM = None
else:
    _color_cams = [c for c in _rs_cams if "color" in c.lower() or "rgb" in c.lower()]
    _depth_cams = [c for c in _rs_cams if "depth" in c.lower() or "ir" in c.lower()]
    REALSENSE_COLOR_CAM = _color_cams[0] if _color_cams else _rs_cams[0]
    REALSENSE_DEPTH_CAM = _depth_cams[0] if _depth_cams else _rs_cams[-1]
    print(f"[spot_standalone] RealSense cameras: color={REALSENSE_COLOR_CAM}, depth={REALSENSE_DEPTH_CAM}")
        ...
        og.Controller.edit(
            {"graph_path": "/World/RealsenseColorGraph", "evaluator_name": "execution"},
            {
                ...
                og.Controller.Keys.SET_VALUES: [
                    ...
                    ("ColorHelper.inputs:frameId", FRAME_REALSENSE),
                ],
                og.Controller.Keys.CONNECT: [
                    ("OnPlaybackTick.outputs:tick", "RunOnce.inputs:execIn"),
                    ("RunOnce.outputs:step", "RenderProduct.inputs:execIn"),
        ...
        og.Controller.edit(
            {"graph_path": "/World/RealsenseDepthGraph", "evaluator_name": "execution"},
            {
                ...
                og.Controller.Keys.SET_VALUES: [
                    ...
                    ("DepthHelper.inputs:frameId", FRAME_REALSENSE),
